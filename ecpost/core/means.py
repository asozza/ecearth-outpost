...
def spacemean(data, ndim, orca='ORCA2'):
    """ 
...
def spacemean_new(data, ndim, orca="ORCA2", subset=None):
    """
    Spatial average of a field using ORCA grid weights.
    ...
    ndim : {"1D", "2D", "3D"}
    orca : str
        ORCA grid resolution
    subset : dict or None
        Optional spatial subset, e.g. {"x": slice(10, 50), "y": slice(20, 100)}

    Returns
    -------
    xr.DataArray or xr.Dataset
        Spatial mean over domain (subsetted or full)
    """
    ...
    if subset is not None:
        ...
        # subset dei pesi
        # (solo le dimensioni che esistono nei pesi)
        w = df["V"] if ndim=="3D" else df["S"] if ndim=="2D" else df["z"]
        w = w.sel({k: v for k, v in subset.items() if k in w.dims})
    else:
        w = df["V"] if ndim=="3D" else df["S"] if ndim=="2D" else df["z"]
    ...
    # Align dimensions (important for ORCA masks)
    w = w.broadcast_like(data, exclude=set(data.dims) - set(dims))

    # Weighted mean (subset already applied if needed)
